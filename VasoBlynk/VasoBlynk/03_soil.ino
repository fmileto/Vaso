...
void soilMoistureMeasure()
{
  ...
  switch (soilMoistureMeasureState)
  {
    ...
    case 3:                                             // after all measurement repetitions have been performed
      ...
      {
        BLYNK_LOG("Calibrating => soilMoistureCalibrationAir = %i", soilMoistureAverage);
        soilMoistureCalibrationAir = soilMoistureAverage;
      }
      if (soilMoistureCalibrationWaterFlag == 1)
      {
        BLYNK_LOG("Calibrating => soilMoistureCalibrationWater = %i", soilMoistureAverage);
        soilMoistureCalibrationWater = soilMoistureAverage;
      }
      soilMoisturePercentage = map(soilMoistureAverage, soilMoistureCalibrationAir, soilMoistureCalibrationWater, 0, 100);                                        // calibrate new value with map function
